    ...
    public void run(){
       ...
       while (c < 9){
        ...
        if (valid == 0){
			...
			System.out.println("You have an error in row: " + (c+1));
			error = 1;
		}

      //in the case of no error in row, the user is notified
        if (c == 8 && valid == 1 && error == 0){
				
			System.out.println("Your rows are free from error!");
		}

        //move to next row and restart validity
